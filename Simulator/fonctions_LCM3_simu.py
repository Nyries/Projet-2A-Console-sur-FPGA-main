    ...
    if count_R==3:
        if len(register_recognition(instruction)[1])!=0:
            for i in range(len(register_recognition(instruction)[1])):
                ...
                error.append(line)
            error_count+=1
        if len(virtual_memory_update(virtual_register[int(register_recognition(instruction)[0][1],2)],virtual_register[int(register_recognition(instruction)[0][0],2)]))>0:
    ...
    if count_R==3:
        if len(register_recognition(instruction)[1])!=0:
            for i in range(len(register_recognition(instruction)[1])):
                ...
                error.append(line)
            error_count+=1
        if type(virtual_memory_read(virtual_register[int(register_recognition(instruction)[0][1],2)]))==str:
